...
/**
 * Shared session body construction for realtime provider handlers.
 *
 * Expects the using class to handle provider-specific differences
 * (e.g., default voice) via the $defaultVoice parameter.
 */
trait BuildsRealtimeBody
{
    /**
     * Build the session configuration body from a RealtimeRequest.
     *
     * @return array<string, mixed>
     */
    private function buildSessionBody(RealtimeRequest $request, ?string $defaultVoice = null): array
    {
        $body = array_filter([
            'model' => $request->model,
     ...
     * @return array<string, mixed>
     */
    private function buildTurnDetection(RealtimeRequest $request): array
    {
