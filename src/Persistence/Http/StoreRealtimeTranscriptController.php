...
/**
 * Package-provided endpoint for storing realtime voice transcript turns.
 *
 * Persists voice turns as regular conversation messages tagged with
 * realtime metadata, so consumers get transcript persistence for free
 * when Atlas persistence is enabled.
 */
class StoreRealtimeTranscriptController
{
    ...
    public function __invoke(Request $request, string $sessionId): JsonResponse
    {
        ...
        foreach ($validated['turns'] as $turn) {
            ...
            // Realtime messages are always considered read
            $stored->update(['read_at' => now()]);
