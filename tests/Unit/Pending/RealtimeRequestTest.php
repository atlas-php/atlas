...
function createRealtimePending(?Driver $driver = null): RealtimeRequest
{
    $driver ??= Mockery::mock(Driver::class);
    $registry = Mockery::mock(ProviderRegistryContract::class);
    $registry->shouldReceive('resolve')->with('openai')->andReturn($driver);

    return new RealtimeRequest('openai', 'gpt-4o-realtime-preview', $registry);
}
...
it('builds request DTO with correct values', function () {
    ...
    expect($request->transport)->toBe(RealtimeTransport::WebSocket);
...
it('dispatches createSession to driver and fires RealtimeSessionCreated', function () {
    Event::fake();
    ...
    $driver = Mockery::mock(Driver::class);
    $driver->shouldReceive('capabilities')->andReturn(new ProviderCapabilities(realtime: true));
    $driver->shouldReceive('createRealtimeSession')->once()->andReturn($session);
    ...
    Event::assertDispatched(
        RealtimeSessionCreated::class,
        fn ($e) => $e->sessionId === 'rt_123' && $e->transport === RealtimeTransport::WebRtc
    );
});
...
it('fires ModalityCompleted on createSession error', function () {
    ...
    $driver = Mockery::mock(Driver::class);
    $driver->shouldReceive('capabilities')->andReturn(new ProviderCapabilities(realtime: true));
    $driver->shouldReceive('createRealtimeSession')->andThrow(new RuntimeException('fail'));
